...
exports.calcularPago = async (req, res) => {
  try {
    ...
    if (trab[0].tipo === 'jornalero') {
      ...
      const sueldoMensual = Number(trab[0].sueldo_mensual) || 0;
      totalPagar = Math.round((sueldoMensual + Number(ventas[0].total_comision) + Number(ventas[0].total_sobreprecio)) * 100) / 100;
      tipoPago = 'mixto';
    }
